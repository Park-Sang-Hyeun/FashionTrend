...
const TrendShoppingSearchCloud = ({ keyword }) => {
  ...
  const saveToWishlist = async (product) => {
    try {
      ...
      axiosInstance.interceptors.request.use((config) => {
        console.log("보내는 토큰:", config.headers.Authorization);
        return config;
    });
  ...
  return (
    <div className="trend-shopping-container">
      <h2 className="trend-shopping-header" style={{ textAlign: 'center', justifyContent: 'center' }}>
      ...
      {/* ToastContainer 추가 */}
      <ToastContainer style={{ width:'370px'}} bodyClassName="custom-toast-body"/>
    </div>
  );
};

export default TrendShoppingSearchCloud;
